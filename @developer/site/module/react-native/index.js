(async function define_module(...x){ const define = async (module, ...inputs)=>await window.modules.define('ReactNative', {value:await module(...inputs)}); return window.modules.has('ReactNative')?window.modules.get('ReactNative'):await (async ([module],asyncs,...inputs)=>await define(module, ...(await Promise.all(asyncs)).concat(inputs)))(x.splice(0, 1),(x=x.map(i=>i instanceof Promise?async ()=>await i:i).reduce((l, i)=>((typeof(i)==='function'&&i.constructor.name==='AsyncFunction')?l[0].push(i()):l.push(i),l),[[]]))[0], ...x.slice(1, x.length)); })
(async function export_module(ReactNative,Page){
	/* eslint-env browser */
	'use strict';
	let worker;
	let queuedMessages = []
	let appExecuted = false


	//exports
	ReactNative.connect = connectToDebuggerProxy
	return ReactNative

	//scope actions
	function connectToDebuggerProxy(){
		const ws = new WebSocket(ReactNative.socket_url)


		function createJSRuntime(){
			// This worker will run the application JavaScript code,
			// making sure that it's run in an environment without a global
			// document, to make it consistent with the JSC executor environment.
			worker = new Worker(ReactNative.worker_url)
			worker.addEventListener('message', function(message){

				try{
					const {variant,input,type} = message.detail.data
					Page.component.add({variant, input, type})

				}
				catch(error){ }
			})
			worker.onmessage = function(message){

				try{
					const detail = message.detail
					if('data' in detail === false) ws.send(JSON.stringify(message.data))
				}
				catch(error){
					console.error(error)
				}


			}
			window.onbeforeunload = function(){
				return 'If you reload this page, it is going to break the debugging session. ' +
					'You should press' + Page.refreshShortcut + 'in simulator to reload.'
			}
			updateVisibility()
		}

		function shutdownJSRuntime(){
			if(worker){
				worker.terminate()
				worker = null
				window.onbeforeunload = null
			}
		}

		function updateVisibility(){
			if(worker && !Page.state.isPriorityMaintained){
				worker.postMessage({
					method: 'setDebuggerVisibility',
					visibilityState: window.document.visibilityState
				})
			}
			Page.setState({visibilityState: window.document.visibilityState})
		}

		ws.onopen = function(){ Page.setState({status: {type: 'connecting'}}) }

		ws.onmessage = async function(message){
			if(!message.data) return
			const object = JSON.parse(message.data)
			if(object.$event === 'client-disconnected'){
				shutdownJSRuntime()
				Page.setState({status: {type: 'disconnected'}})
				return
			}

			if(!object.method) return


			// Special message that asks for a new JS runtime
			if(object.method === 'prepareJSRuntime'){
				shutdownJSRuntime()
				console.clear()
				Page.component.clear()
				createJSRuntime()
				ws.send(JSON.stringify({replyID: object.id}))
				Page.setState({status: {type: 'connected', id: object.id}})
			}
			else if(object.method === '$disconnected'){
				shutdownJSRuntime()
				Page.setState({status: {type: 'disconnected'}})
			}
			else if(object.method === 'executeApplicationScript'){
				appExecuted = false
				worker.postMessage({
					...object,
					url: await getBlobUrl(object.url)
				})
				appExecuted = true
				// Flush any messages queued up and clear them
				for(const message of queuedMessages){
					worker.postMessage(message)
				}
				queuedMessages = []
			}
			else{
				// Otherwise, pass through to the worker provided the
				// application script has been executed. If not add
				// it to a queue until it has been executed.
				if(appExecuted){
					worker.postMessage(object)
				}
				else{
					queuedMessages.push(object)
				}
			}
		}

		ws.onclose = function(error){
			shutdownJSRuntime()
			Page.setState({status: {type: 'error', error}})
			if(error.reason) console.warn(error.reason)
			setTimeout(connectToDebuggerProxy, 500)
		}

		// Let debuggerWorker.js know when we're not visible so that we can warn about
		// poor performance when using remote debugging.
		document.addEventListener('visibilitychange', updateVisibility, false)
	}

	function getBlobUrl(url){
		// Ensure that the bundle URL has the same origin as this webpage so that
		// the same-origin policy lets us fetch it
		const urlObject = new URL(url, ReactNative.debugger_url)
		const relativeUrl = urlObject.pathname.replace('.bundle', '.delta') +
			urlObject.search +
			urlObject.hash
		const localUrl = new URL(relativeUrl, ReactNative.debugger_url).toString()
		return ReactNative.Delta.blob(localUrl)
	}
}, async function define(){
	const index_url = window.HTMLElement.SourceCode.url('react-native/index.js')
	const debugger_port = 8081
	const debugger_url = new URL(`http://localhost:${debugger_port}/debugger-ui`)
	const socket_url = new URL(`ws://localhost:${debugger_port}/debugger-proxy`)
	socket_url.searchParams.set('role', 'debugger')
	socket_url.searchParams.set('name', 'Chrome')
	return window.modules.set('ReactNative',{
		debugger_url,
		index_url,
		socket_url,
		worker_url: index_url.at('worker/debugger.js')
	})

}, async function Page(){
	await window.modules.wait('react-native-console')
	const isMacLike = /(Mac|iPhone|iPod|iPad)/i.test(navigator.platform)
	const refreshShortcut = isMacLike ? '⌘R':'Ctrl R'
	const Assets = {
		blueIcon: 'data:image/png;base64,',
		grayIcon: 'data:image/png;base64,',
		orangeIcon: 'data:image/png;base64,'
	}
	let component = window.document.querySelector('react-native-console')
	if(component === null) window.document.body.appendChild(component = window.modules.element.create('react-native-console', {id: 'console'}))
	let linkNode = window.document.head.querySelector('link[rel=icon]')
	if(linkNode === null) window.document.head.appendChild(linkNode = window.modules.element.create('link', {rel: 'icon'}))

	const Page = {
		isMacLike,
		refreshShortcut,
		state: {
			isDark: window.localStorage.getItem('darkTheme') === 'on',
			isPriorityMaintained: window.localStorage.getItem('maintainPriority') === 'on',
			status: {type: 'disconnected'},
			visibilityState: window.document.visibilityState
		},

		setState(partialState){
			Page.state = Object.assign({}, Page.state, partialState)
			Page.render()
		},
		component,
		render(){
			const {
				isDark,
				isPriorityMaintained,
				status,
				visibilityState
			} = Page.state

			const statusNode = this.component.gui.status
			switch(status.type){
				case 'connected':
					statusNode.innerHTML = 'Debugger session #' + status.id + ' active.'
					break
				case 'error':
					statusNode.innerHTML = status.error.reason || 'Disconnected from proxy. Attempting reconnection. Is node server running?'
					break
				case 'connecting':
				case 'disconnected':
				// Fall through.
				default:
					statusNode.innerHTML = 'Waiting, press <span class="shortcut">' + refreshShortcut + '</span> in simulator to reload and connect.'
					break
			}

			if(status.type === 'disconnected' ||
				status.type === 'error'){
				linkNode.href = Assets.grayIcon
			}
			else{
				if(visibilityState === 'visible' || isPriorityMaintained){
					linkNode.href = Assets.blueIcon
				}
				else{
					linkNode.href = Assets.orangeIcon
				}
			}

			const darkCheckbox = this.component.gui.dark
			this.component.toggleAttribute('dark', isDark)
			darkCheckbox.checked = isDark
			window.localStorage.setItem('darkTheme', isDark ? 'on':'')

			const maintainPriorityCheckbox = this.component.gui['maintain-priority']

			maintainPriorityCheckbox.checked = isPriorityMaintained
			window.localStorage.setItem('maintainPriority', isPriorityMaintained ? 'on':'')
		},

		toggleDarkTheme(){
			Page.setState({isDark: !Page.state.isDark})
		},

		togglePriorityMaintenance(){
			Page.setState({isPriorityMaintained: !Page.state.isPriorityMaintained})
		}

	}
	//exports
	return Page
}, async function load_assets(){
	const url = await window.modules.wait('modules.ReactNative.index_url', true)
	return window.modules.import.assets([
		url.at('Delta.Patcher.js'),
		url.at('Delta.blob.js'),
		url.at('react-native-console.js'),
	])
})