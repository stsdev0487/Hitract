...
import {Area, Card, Gradient, Hitract, Space, Items, CourseDataset} from 'Hitract/Component'
...
export default class Courses extends React.Component{
	state = {loading: true }
	onItem(item){
		this.props.navigation.navigate('Course', {course:item.data})
	}

	async refresh(reload = true){
		try{
			this.setState(await authorizedState(this, ()=>Api.get.courseList({}, {reload})))
		}
		catch(error){
			console.error(error)
...
async function loadStateDataSource(loader){
	let items = null
	try{ items = new CourseDataset(await loader()) }
...
function renderItem({titleSv, titleEn, topics}){
	const count = topics.length
	return `${titleSv || titleEn} - ${count}`
}